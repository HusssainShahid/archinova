...
| ABT-28 | Upload Drawings or Documents (optional) | ≈ Ours | ☑ Tested | Replaced with `fileLink` URL field — our call: paid upload caps (~25MB) are too small for DWG / plan-set PDFs; share link is the workable path. Soft ask left under Q2 if they want a real upload box anyway |
...
| ABT-31 | Form submission actually reaches the client's inbox | ☑ Done | ☑ Tested | Both forms POST to Web3Forms with live access key in `js/main.js`; reply-to set to visitor email. Mailto retained only as fallback when key starts `REPLACE-WITH` |
...
| BUG-12 | Enquiries reached no server: both forms opened the visitor's own email client, so the enquiry only arrived if they pressed Send, and webmail users saw nothing happen at all | ☑ Done | ☑ Tested | Both forms POST to Web3Forms. Live access key in `js/main.js`. Playwright-tested success, failure, validation, honeypot, and unconfigured mailto fallback. Soft leftover under Q2: Privacy Policy should name Web3Forms as processor |
| BUG-13 | The "Upload Drawings or Documents" file input silently discarded every file, because `mailto:` cannot carry attachments | ≈ Ours | ☑ Tested | Replaced on both forms with a "Link to Drawings or Documents" URL field. Our reading: paying for uploads would not fix it either, since every tier caps at 25MB per submission and DWG or plan-set PDFs exceed that. Assertion added so no `type="file"` returns |
