...
| Script | What it does |
|--------|--------------|
| `verify.mjs` | The regression suite. Resolves every local `href`/`src` on all 11 pages and asserts the current design is still in place. **Run this after any edit.** Exits non-zero on failure. |
...
| `check-refs.mjs` | Cross-checks `requirements/v2` and `requirements/v3` checklist ↔ questions pairs: catches a checklist row pointing at a question that no longer exists. |
...
`mark-questions.mjs` is additionally obsolete: `requirements/v2/V2-QUESTIONS.md` was later
restructured from 32 questions to 8 and the script's expected format no longer matches.
